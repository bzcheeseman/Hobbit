...
namespace { // Maybe wait until we use the tensors a little bit to test them...
  ...
  TEST(Shape, AtCtx) {
    ...
    for (auto iter = BB->begin(), end = BB->end(); iter != end; ++iter) {
      ...
      if (ret != nullptr) {
        retval = llvm::dyn_cast<llvm::ConstantInt>(ret->getOperand(0))->getZExtValue();
      }
    }
  ...
  ; // TODO: add tests for Tensor (some small ones for now, not a lot of codegen to be done?)
