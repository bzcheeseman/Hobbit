...
Hobbit::codegen::Function Hobbit::codegen::Visitor::GetWrapperFunction(const std::string &name) {
  llvm::Type *ret_type = (*m_ops_.end())->GetOp()->GetOutputType();
  Function f;
  f.name = name;
  std::vector<llvm::Type *> arg_types;
  for (auto &arg : m_args_) {
    arg_types.push_back(arg->GetType()->getPointerTo());
  }
  arg_types.push_back(ret_type->getPointerTo());
