...
Hobbit::internal::Shape::Shape(llvm::LLVMContext &ctx,
...
Hobbit::internal::Shape::Shape(llvm::ArrayRef<uint64_t> dims)
    : m_dims_(dims.begin(), dims.end()), m_has_llvm_(false) {}

Hobbit::internal::Shape::Shape(llvm::ArrayRef<llvm::Value *> dims)
...
void Hobbit::internal::Shape::InitLLVM(llvm::LLVMContext &ctx) {
...
uint64_t Hobbit::internal::Shape::NDim() const { return m_dims_.size(); }

uint64_t Hobbit::internal::Shape::Dim(uint64_t which) const {
  CHECK_LT(which, m_dims_.size());
  return m_dims_[which];
}

llvm::Value *Hobbit::internal::Shape::Dim(llvm::Value *which) const {
...
uint64_t Hobbit::internal::Shape::Size() const {
  return std::accumulate(m_dims_.begin(), m_dims_.end(), (uint64_t)1,
                         std::multiplies<uint64_t>());
}

llvm::Value *Hobbit::internal::Shape::Size(llvm::BasicBlock *BB) const {
  CHECK(m_has_llvm_) << "LLVM Value dims not initialized";
...
uint64_t Hobbit::internal::Shape::At(llvm::ArrayRef<uint64_t> idx) const {
...
llvm::Value *Hobbit::internal::Shape::At(llvm::ArrayRef<llvm::Value *> idx,
...
Hobbit::internal::Shape Hobbit::internal::Shape::Flatten(llvm::BasicBlock *BB) const {
  if (!m_has_llvm_ || BB == nullptr) {
    llvm::ArrayRef<uint64_t> flattened_dims = {this->Size()};
    return Hobbit::internal::Shape(flattened_dims);
  }
  CHECK_NOTNULL(BB);
