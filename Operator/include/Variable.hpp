...
namespace Hobbit {
  ...
  struct Range {
    Range(const uint32_t &start = 0, const uint32_t &end = 0)
        : start(start), end(end) {}
  ...
  public:
    ...
    // Pack and Split
    Variable(llvm::IRBuilder<> &builder, llvm::Type *scalar_type,
             const Shape &shape);

    Variable(llvm::Value *value, llvm::Type *type, const Shape &shape,
             Variable *parent);
    ...
    // split if desired.
    Variable *GetChunk(llvm::IRBuilder<> &builder, const Range &k_range,
