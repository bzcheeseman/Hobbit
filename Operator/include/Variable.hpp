...
//  llvm::Intrinsic:: is the namespace for intrinsics
  ...
  public:
    ...
    // desired.
    llvm::ArrayRef<Variable> Split(llvm::IRBuilder<> &builder, uint32_t chunk_size);

    // Accept llvm::Pointer/llvm::Array

  protected:
    llvm::ArrayRef<llvm::Value *> PackPtrToVector_(llvm::IRBuilder<> &builder, llvm::Type *vector_type);
    llvm::ArrayRef<llvm::Value *> PackArrayToVector_(llvm::IRBuilder<> &builder, llvm::Type *vector_type);

    llvm::ArrayRef<Variable> PackPtrToVariable_(llvm::IRBuilder<> &builder, llvm::Type *array_type);
    llvm::ArrayRef<Variable> PackArrayToVariable_(llvm::IRBuilder<> &builder, llvm::Type *array_type);
  ...
  public:
    ...
    // As a general rule of thumb this should be used to split the object being wrapped into sse/avx style
    // vectors
    llvm::ArrayRef<llvm::Value *> SplitVector(llvm::IRBuilder<> &builder, uint32_t chunk_size);
